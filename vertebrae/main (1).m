...
finfs = cell(length(subjects), 1); % percentage of points that describe inferior surfaces
fsups = cell(length(subjects), 1); % percentage of points that describe superior surfaces
...
for ii = 1:length(subjects)
    ...
    finfs{ii} = cell(numLevels(ii), 1); % percentage of points that describe inferior surfaces of subject ii
    fsups{ii} = cell(numLevels(ii), 1); % percentage of points that describe superior surfaces of subject ii
...
for ii = 1:length(subjects)
    for jj = 1:numLevels(ii)
        ...
        measureVertebraeArea; % returns areaZ, Z; area and slice position measurements
