...
% identifying inferior + superior surface index locations, inf_ind + sup_ind:
p_sup = 0.0525; % increasing this parameter, pushes superior landmark up
p_inf = 0.05; % increasing this parameter, pushes inferior landmark down
sign_sup = sign(dcsadh + p_sup * max(dcsadh));
sign_inf = sign(dcsadh - p_inf * max(dcsadh));
diff_sup = diff(sign_sup);
diff_inf = diff(sign_inf);
indices_sup = find(diff_sup ~= 0) + 1; % Add 1 to account for diff shifting by 1
indices_inf = find(diff_inf ~= 0) + 1; % Add 1 to account for diff shifting by 1
sup_ind = indices_sup(end);
inf_ind = indices_inf(1);

% percentage of points that describe inferior and superior surfaces:
finf = Znorm(inf_ind);
fsup = 1 - Znorm(sup_ind);
...
if makeplot
    ...
    % normalized cross sectional area VS slice position
    subplot(1,5,3);
    plot(areaZnorm, Znorm)
    yline(Znorm(inf_ind), '--k') 
    yline(Znorm(sup_ind), '--k')
    xlabel('normalized csa')
    ylabel('normalized height')
    title('Normalized area for ' + figtitle)
    drawnow
    ...
    for sl = 1:2
        ...
        if makeplot
            ec = 'k';
            % display the object and slice plane
            title("Inferior index = " + string(inf_ind) + ", superior index = " + string(sup_ind))
            hp{1} = patch(model,'facecolor','w','edgecolor',ec); hold on
            hp{2} = patch(plane,'facecolor','m','edgecolor',ec,'facealpha',0.5);
            followerlight(hp{1});
            axis equal
...
varskeep = {'areaZ', 'Z', 'finf', 'fsup', 'infPV', 'supPV', 'afig', 'ps', 'sup_ind', 'inf_ind'};
